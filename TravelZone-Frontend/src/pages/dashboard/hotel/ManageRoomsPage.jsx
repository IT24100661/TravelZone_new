...
import { BedDouble, DollarSign, Save } from "lucide-react";
...
function ManageRoomsPage() {
  ...
  useEffect(() => {
    api.get("/api/hotels").then((res) => setHotels(res.data.content || [])).catch(() => {});
  }, []);
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-slate-800 mb-6">Manage Rooms</h1>

      {/* Select hotel */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 mb-6">
        <label className="block text-sm font-medium text-slate-700 mb-2">Select Hotel</label>
        <select
          onChange={(e) => loadRooms(parseInt(e.target.value))}
      ...
      {rooms.length === 0 && selectedHotel && (
        <div className="bg-white rounded-3xl border border-slate-200 p-16 text-center">
          ...
          <p className="text-slate-500 font-semibold">No rooms found for this hotel</p>
        </div>
      )}
          ...
          return (
            <div key={room.roomId} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`px-3 py-1 rounded-full text-xs font-semibold border ${room.available ? "bg-emerald-100 text-emerald-700 border-emerald-200" : "bg-red-100 text-red-600 border-red-200"}`}>
