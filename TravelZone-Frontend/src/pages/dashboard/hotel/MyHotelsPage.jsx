...
import { Building2, MapPin, Plus, X, Star } from "lucide-react";
...
function MyHotelsPage() {
  ...
  const removeItem = (field, val) =>
    setForm((p) => ({ ...p, [field]: p[field].filter((v) => v !== val) }));

  const handleSubmit = async (e) => {
    e.preventDefault();
    setError(""); setSuccess("");
    if (form.images.length === 0) { setError("Add at least one image URL"); return; }
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div>
              ...
              <textarea name="description" value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))}
                required rows={3} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-blue-400 transition resize-none"
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <input value={facilityInput} onChange={(e) => setFacilityInput(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addItem("facilities", facilityInput, setFacilityInput))}
                ...
                <button type="button" onClick={() => addItem("facilities", facilityInput, setFacilityInput)}
              ...
              <div className="flex flex-wrap gap-2">
                {form.facilities.map((f) => (
                  <span key={f} className="flex items-center gap-1.5 bg-blue-50 text-blue-700 border border-blue-200 px-3 py-1 rounded-full text-xs font-medium">
                    {f} <button type="button" onClick={() => removeItem("facilities", f)}><X size={11} /></button>
                  </span>
                ))}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2 mb-2">
                <input value={imageInput} onChange={(e) => setImageInput(e.target.value)}
                  onKeyDown={(e) => e.key === "Enter" && (e.preventDefault(), addItem("images", imageInput, setImageInput))}
                  className="flex-1 border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:border-blue-400 transition"
                  placeholder="https://example.com/hotel.jpg" />
                <button type="button" onClick={() => addItem("images", imageInput, setImageInput)}
                  className="bg-blue-600 hover:bg-blue-500 text-white px-4 rounded-xl transition">
                  <Plus size={16} />
                </button>
              </div>
              <div className="flex flex-wrap gap-2">
                {form.images.map((img) => (
                  <span key={img} className="flex items-center gap-1.5 bg-slate-100 text-slate-600 border border-slate-200 px-3 py-1 rounded-full text-xs truncate max-w-xs">
                    {img} <button type="button" onClick={() => removeItem("images", img)}><X size={11} /></button>
                  </span>
                ))}
              </div>
            </div>
          ...
          {hotels.map((hotel) => (
            <div key={hotel.hotelId} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition overflow-hidden">
              ...
              <div className="p-4">
                ...
                <p className="text-slate-400 text-xs flex items-center gap-1 mt-1"><MapPin size={11} /> {hotel.location}</p>
                <div className="flex items-center justify-between mt-2">
                  ...
                  <span className="text-emerald-600 text-xs font-bold">${parseFloat(hotel.minPrice || 0).toFixed(0)}/night</span>
                </div>
              </div>
            </div>
          ))}
